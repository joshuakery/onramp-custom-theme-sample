...
<ul class="mec-weekly-view-dates-events">
    ...
    <li class="mec-weekly-view-date-events mec-util-hidden mec-calendar-day-events mec-clear mec-weekly-view-week-<?php echo $this->id; ?>-<?php echo date('Ym', strtotime($date)).$week; ?>" id="mec_weekly_view_date_events<?php echo $this->id; ?>_<?php echo date('Ymd', strtotime($date)); ?>" data-week-number="<?php echo $week; ?>">
        ...
        <?php $events = apply_filters( 'orc_filter_events', $events ); ?>
        ...
        <?php foreach($events as $event): ?>
            ...
            <article data-style="<?php echo $label_style; ?>" class="<?php echo (isset($event->data->meta['event_past']) and trim($event->data->meta['event_past'])) ? 'mec-past-event ' : ''; ?>mec-event-article <?php echo $this->get_event_classes($event); ?> wheel_color_<?php echo orc_get_the_parent_category($event)->slug;  ?>">
                ...
                <p class="mec_event_wheel_cat"><?php echo orc_get_the_parent_category($event)->name;  ?></p>
                <p class="mec_event_subject"><?php echo orc_get_the_child_category($event)->name;  ?></p>
                <?php
                $event_tags = get_the_tags(get_post($event));
                if ($event_tags) {
                    foreach ($event_tags as $tag) {
                        echo '<p class="mec_event_tag">' . $tag->name . '</p>';
                    }
                }
                ?>
...
<?php
function orc_get_the_parent_category($e) {
    $post = get_post($e->ID);
...
function orc_get_the_child_category($event) {
...
function orc_get_the_tags($event) {
    $post = get_post($event->ID);
